{ lib, stdenv, cmake, git, apple-sdk_11, ninja, fetchFromGitHub, SDL2, wget
, which, autoAddDriverRunpath, makeWrapper, nix-update-script,
...
vulkanSupport ? true, shaderc, vulkan-headers, vulkan-loader,
...
let
  ...
  inherit (lib) cmakeBool cmakeFeature optional optionals;
  ...
  vulkanBuildInputs = [ shaderc vulkan-headers vulkan-loader ];
...
in effectiveStdenv.mkDerivation (finalAttrs: {
  ...
  nativeBuildInputs = [ cmake git ninja which makeWrapper ]
    ++ lib.optionals cudaSupport [
      cudaPackages.cuda_nvcc
      autoAddDriverRunpath
    ];

  buildInputs = optional withSDL SDL2
  ...
  ] ++ optionals (effectiveStdenv.hostPlatform.isx86
    && !effectiveStdenv.hostPlatform.isStatic) [
      (cmakeBool "GGML_BACKEND_DL" true)
      (cmakeBool "GGML_CPU_ALL_VARIANTS" true)
    ] ++ optionals cudaSupport [
      (cmakeFeature "CMAKE_CUDA_ARCHITECTURES"
        cudaPackages.flags.cmakeCudaArchitecturesString)
  ...
  requiredSystemFeatures = optionals rocmSupport [
    "big-parallel"
  ]; # rocmSupport multiplies build time by the number of GPU targets, which takes arround 30 minutes on a 16-cores system to build
  ...
  meta = {
    ...
    maintainers = with lib.maintainers; [ dit7ya hughobrien aviallon ];
  };
})
