{pkgs, ...}: {
  programs.fish = {
    ...
    shellAliases = {
      ...
      tuivpnon = "nmcli radio wifi on || true && sleep 5 && echo \"Check Yubikey\" && sudo nmcli device disconnect enp38s0 || true  && nmcli device connect wlo1 || true && nmcli device connect enp38s0 || true && sudo ip route add 10.141.96.8/32 via 192.168.137.1 dev wlo1 && sudo ip route add 10.141.96.194/32 via 192.168.137.1 dev wlo1";
