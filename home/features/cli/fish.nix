{ pkgs, ... }: {
  programs.fish = {
    ...
    shellAliases = {
      ...
      gpgFrontend = ''
        QT_QPA_PLATFORM="wayland;xcb" appimage-run /home/gogsaan/Applications/appimage/GpgFrontend-2.0.9-linux-x86_64_4a0a7c2d621c582a773f4a7652633cc3.AppImage'';
      lightDark = ''
        if grep -q 'Catppuccin-Mocha-Standard-Mauve-Dark' /home/gogsaan/Projects/private/nix/config/home/features/desktop/common/wayland/gtk.nix; sed -i 's/Catppuccin-Mocha-Standard-Mauve-Dark/Catppuccin-Latte-Standard-Mauve-Light/g' /home/gogsaan/Projects/private/nix/config/home/features/desktop/common/wayland/gtk.nix; else if grep -q 'Catppuccin-Latte-Standard-Mauve-Light' /home/gogsaan/Projects/private/nix/config/home/features/desktop/common/wayland/gtk.nix; sed -i 's/Catppuccin-Latte-Standard-Mauve-Light/Catppuccin-Mocha-Standard-Mauve-Dark/g' /home/gogsaan/Projects/private/nix/config/home/features/desktop/common/wayland/gtk.nix; end; and echo "Check Yubikey" && sudo nixos-rebuild switch --flake /home/gogsaan/Projects/private/nix/config#gza --show-trace'';
      tuivpnon = ''
        nmcli radio wifi on || true && sleep 5 && echo "Check Yubikey" && sudo nmcli device disconnect enp38s0 || true  && nmcli device connect wlo1 || true && nmcli device connect enp38s0 || true && sudo ip route add 10.141.96.190/32 via 192.168.137.1 dev wlo1 && sudo ip route add 10.141.96.117/32 via 192.168.137.1 dev wlo1'';
      ...
      nb = ''
        echo "Check Yubikey" && sudo rm /home/gogsaan/.config/mimeapps.list || true && sudo mv /persist/home/gogsaan/.config/mimeapps.list /persist/home/gogsaan/.config/mimeapps1.list || true && sudo rm /persist/home/gogsaan/.ssh/config || true && git add . || true && sudo nixos-rebuild switch --show-trace --option eval-cache false --flake /home/gogsaan/Projects/private/nix/config#gza --show-trace && sudo mv /persist/home/gogsaan/.config/mimeapps1.list /persist/home/gogsaan/.config/mimeapps.list || true && ln -s /persist/home/gogsaan/.config/mimeapps.list /home/gogsaan/.config/mimeapps.list || true'';
      ...
      nu = ''
        echo "Check Yubikey" && cd /persist/home/gogsaan/Projects/private/nix/config  && sudo nix flake update'';
      ...
      kubefwd = ''
        echo "Check Yubikey" && sudo cp /etc/hosts /etc/hostsTemp && sudo rm /etc/hosts && sudo cp /etc/hostsTemp /etc/hosts && sudo ~/Applications/bin/kubefwd svc -c $KUBECONFIG -n crawlyfi-prod'';
      ...
      protonge = ''
        protonup -d "/home/gogsaan/.steam/steam/compatibilitytools.d" && protonup '';
      ...
      gitNotMegedProduction = ''
        for branch in (git for-each-ref --format='%(refname:short)' refs/heads/ refs/remotes/origin/ | grep -v 'development' | grep -v 'production'); if git merge-base --is-ancestor $branch development && not git merge-base --is-ancestor $branch production; echo "$branch has been merged into development but not into production"; end; end;'';
      mountBackup = ''
        echo "Check Yubikey" && sudo lvchange -ay volgroup_mirror/backup && sudo mount /dev/volgroup_mirror/backup /persist/drivers/backup/  && sudo lvchange -ay volgroup_mirror/backup && sudo mount /dev/volgroup_mirror/backup /persist/drivers/backup/'';
    ...
    functions = {
      ...
      "cursor-agent" = {
        description = "Run Cursor agent using Nix Node";
        body = ''
          set -l dir (dirname (realpath ~/.local/bin/cursor-agent))
          nix shell nixpkgs#nodejs_22 -c node $dir/index.js $argv
        '';
      };
    };
    interactiveShellInit =
      ...
      ''
        # Detect if Kiro terminal is running and switch to bash
